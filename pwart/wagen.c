...
static int pwart_EmitCallFunc(Module *m, Type *type, sljit_s32 memreg,
  ...
  if ((memreg & (SLJIT_MEM-1)) == SLJIT_R0 || (memreg & (SLJIT_MEM-1)) == SLJIT_R1) {
...
static int pwart_EmitFuncEnter(Module *m) {
  ...
  // for performance, put memory base to SLJIT_S1, and save RuntimeContext pointer. Must be the last step.
...
static int pwart_GenCode(Module *m) {
  ...
  while (!eof && m->pc < m->byte_count) {
    ...
    case 0x0e: // br_table
      ...
      for (a = 0; a < count; a++) {
        if (a == 0) {
          sljit_emit_cmp(m->jitc, SLJIT_EQUAL, sv->val.op, sv->val.opw,
                         SLJIT_IMM, 0);
        } else {
